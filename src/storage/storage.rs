...
impl StorageManager {
    ...
    pub fn create_rel(&self, rel: Rel) -> Result<bool> {
        let catalog_fl_pth = format!("{}/{}", self.db_pth, CATALOG_FILE_NAME);
        let data = serde_json::ser::to_vec(&rel)?;
        FileManager::append_to_file(&catalog_fl_pth, data)?;
        Ok(true)
    }

    pub fn delete_rel(&self, rel_name: &str) -> Result<bool> {
        let catalog_fl_pth = format!("{}/{}", self.db_pth, CATALOG_FILE_NAME);
        ...
        if let Some(pos) = catalog.iter().position(|rel| rel.nm == rel_name) {
            ...
            FileManager::write_to_file(&catalog_fl_pth, 0, data)?;
            Ok(true)
        } else {
            Ok(false)
        }
